...
const PostSelector = props => {
	...
	const buttonText = changeSaveContentButtonText
		? __('Update Content Lineup', 'infobeans')
		: __('Save Content Lineup', 'infobeans');
					...
					<div className="searchbox">
						<label htmlFor="searchinput">
							...
							<input
								id="searchinput"
								type="search"
								placeholder={__('Please enter your search query...', 'infobeans')}
								value={filter}
				...
				{enableLineup && !hideLineupsHeader && (
					<div className="post-selectorHeader">
						<div className="dropdown-checkbox-container">
							<div className="lineup-dropdown-container">
								...
								<div className="select-box-field">
									<label htmlFor="options">{__('Lineup:', 'infobeans')}</label>
									<select name="options" id="lineupOptions" onChange={onLineupChange}>
										<option key="" value="">
											{__('New Lineup', 'infobeans')}
										</option>
										{types.length < 1 ? (
											<option value="">{__('Loading...', 'infobeans')}</option>
										) : (
											lineups &&
											lineups.map(obj => (
												<option
							...
							{siteData.isEditionSupported && (
								<div className="checkbox-container">
									<CheckboxControl
										id="checkbox"
										label={__('Show lineup for all Edition', 'infobeans')}
						...
						<div className="content-lineup-save-button-container">
							...
							>
								{savingLineup ? __('Saving', 'infobeans') : buttonText}
							</button>
						</div>
					</div>
				)}
