...
function setup_node() {
  # Node.js/NVM
  ## Install Node.js/NVM
  if curl -s -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.38.0/install.sh | bash > /dev/null 2>&1; then
    echo "Installed nvm.";
  else
  ...
  if nvm install 18.16.0 > /dev/null 2>&1; then
...
function setup_yarn() {
  # install yarn
  if curl -s -o- -L https://yarnpkg.com/install.sh | bash -s -- --version 1.22.11 > /dev/null 2>&1; then
